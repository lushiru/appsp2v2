...
import { styles } from "./CheckListVerFotosScreen.styles"
import { checklistfotoCtrl } from '../../../api'


export function CheckListVerFotosScreen(props) {
    ...
    const {
      ...
      const getFotos = async (id) => {
        ...
        try {
            const response = await checklistfotoCtrl.verChecklistFoto(id);
      ...
      const goToEliminar = (idf) => {
            ...
            const eliminando = async (idf) => {
              try {
                await checklistfotoCtrl.deleteChecklistFoto(idf);
                getFotos(id);
              }catch(error){
                ToastAndroid.show( "Error " + error , ToastAndroid.SHORT);
              }  
